...
int main (int argc, char **argv)
{
	...
	status = mb_defaults(verbose,&input_swath_format,&pings,&lonflip,bounds,
	  ...
	  switch (c)
		...
		case 0:
			...
			/* input platform file */
				...
				status = mb_platform_read(verbose, input_platform_file,
										  (void **)&platform, &error);
					...
					for (i=0;i<platform->num_sensors;i++)
						...
						for (j=0;j<NUM_MB_SENSOR_TYPES;j++)
							if (mb_sensor_type_id[j] == platform->sensors[i].type)
								index = j;
						fprintf(stderr, "    platform->sensors[%d].type:                 %d <%s>\n", i, platform->sensors[i].type, mb_sensor_type_string[index]);
			...
			/* input swath file or datalist */
				...
				if (input_swath_format < 0)
					...
					if ((status = mb_datalist_open(verbose, &datalist,
									input_swath_file, look_processed, &error)) != MB_SUCCESS)
						{
						error = MB_ERROR_OPEN_FAIL;
				...
				while (read_data == MB_YES
					...
					if ((status = mb_read_init(
						verbose,swath_file,input_swath_format,pings,lonflip,bounds,
						btime_i,etime_i,speedmin,timegap,
			...
			/* sensor-offsets */
				...
				sscanf(optarg, "%lf/%lf/%lf/%lf/%lf/%lf",
					   &tmp_offsets[tmp_sensor.num_offsets].position_offset_x,
			...
			/* sensor-time-latency */
				...
				{
				sscanf(optarg, "%lf",
					   &tmp_sensor.time_latency_static);
			...
			/* sensor-time-latency-model */
				...
				if (tmp_sensor.num_time_latency > tmp_sensor.num_time_latency_alloc)
					...
					status = mb_mallocd(verbose, __FILE__, __LINE__,
										tmp_sensor.num_time_latency * sizeof(double),
				...
				while ((result = fgets(buffer,MB_PATH_MAXLINE,tfp)) == buffer)
					...
					if (buffer[0] != '#')
						...
						if (sscanf(buffer,"%lf %lf",
								   &tmp_sensor.time_latency_time_d[tmp_sensor.num_time_latency],
			...
			/* end-sensor */
				...
				if (sensor_mode == SENSOR_ADD)
					...
					status = mb_platform_add_sensor(verbose, (void *)platform,
													tmp_sensor.type,
													tmp_sensor.model,
													tmp_sensor.manufacturer,
													tmp_sensor.serialnumber,
													tmp_sensor.capability1,
													tmp_sensor.capability2,
													tmp_sensor.num_offsets,
													tmp_sensor.num_time_latency,
													&error);
					for (ioffset=0; ioffset<tmp_sensor.num_offsets; ioffset++)
						...
						status = mb_platform_set_sensor_offset(verbose, (void *)platform,
														sensor_id,
														ioffset,
														tmp_offsets[ioffset].position_offset_mode,
														tmp_offsets[ioffset].position_offset_x,
														tmp_offsets[ioffset].position_offset_y,
														tmp_offsets[ioffset].position_offset_z,   
														tmp_offsets[ioffset].attitude_offset_mode,
														tmp_offsets[ioffset].attitude_offset_heading,
														tmp_offsets[ioffset].attitude_offset_roll,
														tmp_offsets[ioffset].attitude_offset_pitch,
														&error);
						}
					status = mb_platform_set_sensor_timelatency(verbose, (void *)platform,
													sensor_id,
													tmp_sensor.time_latency_mode,
													tmp_sensor.time_latency_static,
													tmp_sensor.num_time_latency,
													tmp_sensor.time_latency_time_d,
													tmp_sensor.time_latency_value,
													&error);
			...
			/* modify-offsets */
				...
				if (sensor_mode == SENSOR_MODIFY || sensor_mode == SENSOR_ADD)
					{
					nscan = sscanf(optarg, "%d/%lf/%lf/%lf/%lf/%lf/%lf",
						   &ioffset, &d1, &d2, &d3, &d4, &d5, &d6);
					if (nscan == 7 && ioffset >= 0
						&& ioffset < active_sensor->num_offsets)
						{
			...
			/* modify-offset-positions */
				...
				if (sensor_mode == SENSOR_MODIFY || sensor_mode == SENSOR_ADD)
					{
					nscan = sscanf(optarg, "%d/%lf/%lf/%lf",
						   &ioffset, &d1, &d2, &d3);
					if (nscan == 4 && ioffset >= 0
						&& ioffset < active_sensor->num_offsets)
						{
						active_sensor->offsets[ioffset].position_offset_x = d1;
						active_sensor->offsets[ioffset].position_offset_y = d2;
			...
			/* modify-time-latency */
				...
				if (sensor_mode == SENSOR_MODIFY || sensor_mode == SENSOR_ADD)
					{
					sscanf(optarg, "%lf",
						   &active_sensor->time_latency_static);
			...
			/* modify-time-latency-model */
				...
				if (sensor_mode == SENSOR_MODIFY || sensor_mode == SENSOR_ADD)
					...
					if (active_sensor->num_time_latency > active_sensor->num_time_latency_alloc)
						{
						status = mb_mallocd(verbose, __FILE__, __LINE__,
											active_sensor->num_time_latency * sizeof(double),
					...
					while ((result = fgets(buffer,MB_PATH_MAXLINE,tfp)) == buffer)
						...
						if (buffer[0] != '#')
							...
							/* read the time and time latency pair */
							if (sscanf(buffer,"%lf %lf",
									   &active_sensor->time_latency_time_d[active_sensor->num_time_latency],
	...
	if (output_platform_file_defined == MB_YES
		&& platform != NULL
